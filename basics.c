...
void transcribe_array_int(array_int *src, array_int *dest)
{
  ...
  (*dest).len = (*src).len;
  for (int i = 0; i < (*src).len; i++) (*dest).e[i] = (*src).e[i];
}
...
void free_array_int(void *av)
{
  array_int *a = (array_int *) av;
  if ((*a).mem > 0)
    {
      if ((*a).e != NULL) free((*a).e);
      (*a).e = NULL;
...
void merge_array_int_rec(array_int *a, int *e, int *c, int imin, int imax, int jmin, int jmax)
{
  if ((*a).e[imin] <= (*a).e[jmin]) {}
  else
    {
      printf("INCORRECT USAGE! AAAAAAHHHH!\n");
      exit(EXIT_FAILURE);
    }
  int il = imin;
  while ((il < imax) && ((*a).e[il] <= (*a).e[jmin]))
    {
      e[(*c)] = (*a).e[il];
      il += 1;
      (*c) += 1;
    }
  if (il < imax) merge_array_int_rec(a, e, c, jmin, jmax, il, imax);
  else
    {
      while (jmin < jmax)
	{
...
{
  if ((*a).e[imin] <= (*a).e[jmin]) merge_array_int_rec(a, e, c, imin, imax, jmin, jmax);
  else merge_array_int_rec(a, e, c, jmin, jmax, imin, imax);
}
  ...
  if (jmi > 2)
    {
      ...
      for (int ii = i; ii < j; ii++) (*a).e[ii] = e[ii];
      int iim1 = i;
      for (int ii = i + 1; ii < j; ii++)
	{
	  if (e[ii] >= e[iim1]) {}
	    ...
	    {
	      printf("Inconsistency found in interval %d %d:\n", i, j);
	      for (int iii = i; iii < j; iii++) printf("%d ", e[iii]);
	      printf("\n");
	      exit(EXIT_FAILURE);
	    }
	  iim1 = ii;
...
void sort_array_int_safe(array_int *a)
{
  char status = is_sorted_array_int(a);
  if (status == 0)
    {
      int *e = malloc(sizeof(int) * (*a).len);
      for (int i = 0; i < (*a).len; i++) e[i] = (*a).e[i];
      merge_sort_array_int(a, e, 0, (*a).len);
      for (int i = 0; i < (*a).len; i++) (*a).e[i] = e[i];
      free(e);
    }
  else if (status == SORTED_DECREASING)
    {
      int hlength = (*a).len >> 1;
      int ii = (*a).len;
...
void sort_array_int(array_int *a)
{
  ...
  if (status == 0)
    {
      int *e = (int *) calloc((*a).len, sizeof(int));
      merge_sort_array_int(a, e, 0, (*a).len);
      if ((*a).mem > 0) free((*a).e);
      (*a).e = e;
      (*a).mem = (*a).len;
...
void free_aarray_int(void *aav)
{
  aarray_int *aa = (aarray_int *) aav;
  if ((*aa).mem > 0)
    {
      int i = 0;
      while (i < (*aa).len)
...
void free_edge_wtd_graph(void *gv)
{
  edge_wtd_graph *g = (edge_wtd_graph *) gv;
...
{
  if (size_ > 0) (*a).e = (double *) calloc(size_, sizeof(double));
  else (*a).e = NULL;
  (*a).mem = size_;
  (*a).len = 0;
}
...
{
  array_double_init(dest, (*src).len);
  (*dest).len = (*src).len;
  for (int i = 0; i < (*src).len; i++) (*dest).e[i] = (*src).e[i];
}
...
void merge_array_double_rec(array_double *a, double *e, int *c, int imin, int imax, int jmin, int jmax)
{
  if ((*a).e[imin] <= (*a).e[jmin]) {}
  else
    {
      printf("Incorrect usage! Wait... BLEAAARGGHH!\n");
      exit(EXIT_FAILURE);
    }
  int il = imin;
  ...
  if (il < imax) merge_array_double_rec(a, e, c, jmin, jmax, il, imax);
  else
    {
      int ir = jmin;
      while (ir < jmax)
...
{
  if ((*a).e[imin] <= (*a).e[jmin]) merge_array_double_rec(a, e, c, imin, imax, jmin, jmax);
  else merge_array_double_rec(a, e, c, jmin, jmax, imin, imax);
}
...
{
  for (int i = 0; i < a.len; i++) printf("%c ", a.e[i]);
  printf("\n");
}
  ...
  if ((*a).mem > 0 && (*a).e != NULL) 
    {
      ...
      int ulim = (*a).len < newsize ? (*a).len : newsize;
      for (int i = 0; i < ulim; i++) data[i] = (*a).e[i];
      free((*a).e);
      (*a).e = data;
      (*a).mem = newsize;
    }
...
void merge_array_char_rec(array_char *a, char *e, int *c, int imin, int imax, int jmin, int jmax)
{
  if ((*a).e[imin] <= (*a).e[jmin]) {}
  else
    {
  ...
  if (il < imax) merge_array_char_rec(a, e, c, jmin, jmax, il, imax);
  else
    {
      int ir = jmin;
      while (ir < jmax)
...
{
  if ((*a).e[imin] <= (*a).e[jmin]) merge_array_char_rec(a, e, c, imin, imax, jmin, jmax);
  else merge_array_char_rec(a, e, c, jmin, jmax, imin, imax);
}
...
void transcribe_aarray_char(aarray_char *src, aarray_char *dest)
{
  ...
  (*dest).len = (*src).len;
  for (int i = 0; i < (*src).len; i++) transcribe_array_char(&((*src).e[i]), &((*dest).e[i]));
}
...
void aarray_char_init_precise(aarray_char *aa, int mem1, int mem2)
{
  ...
  for (int i = 0; i < (*aa).mem; i++) array_char_init(&(*aa).e[i], mem2);
  (*aa).len = 0;
}
  ...
  if ((*a).mem > 0 && (*a).e != NULL)
    {
      ...
      int ulim = (*a).len < newsize ? (*a).len : newsize;
      for (int i = 0; i < ulim; i++) data[i] = (*a).e[i];
      for (int i = ulim; i < newsize; i++) data[i] = (*a).e[i];
      free((*a).e);
      (*a).e = data;
      (*a).mem = newsize;
    }
...
{
  if ((*aa).len == (*aa).mem) add_mem_aarray_char(aa);
  if ((*aa).e[(*aa).len].e != NULL) (*aa).e[(*aa).len].len = 0;
  else array_char_init(&((*aa).e[(*aa).len]), init_mem);
  (*aa).len += 1;
}
...
void merge_aarray_int_rec(aarray_int *a, array_int *e, int *c, int imin, int imax, int jmin, int jmax, char (*order)(array_int *, array_int *))
{
  if (order(&(*a).e[imin], &(*a).e[jmin]) <= 0) {}
  else
    {
      printf("Incorrect usage: merge_aarray_int_rec\n");
      exit(EXIT_FAILURE);
    }
  int il = imin;
  while (order(&(*a).e[il], &(*a).e[jmin]) <= 0 && il < imax)
    {
      e[*c] = (*a).e[il];
      il += 1;
      *c += 1;
    }
  if (il < imax) merge_aarray_int_rec(a, e, c, jmin, jmax, il, imax, order);
  else
    {
      int ir = jmin;
      while (ir < jmax)
...
{
  if (order(&(*a).e[imin], &(*a).e[jmin]) <= 0) merge_aarray_int_rec(a, e, c, imin, imax, jmin, jmax, order);
  else merge_aarray_int_rec(a, e, c, jmin, jmax, imin, imax, order);
}
...
void merge_aarray_double_rec(aarray_double *a, array_double *e, int *c, int imin, int imax, int jmin, int jmax, char (*order)(array_double *, array_double *))
{
  if (order(&(*a).e[imin], &(*a).e[jmin]) <= 0) {}
  else
    {
      printf("Incorrect usage: merge_aarray_double_rec\n");
      exit(EXIT_FAILURE);
    }
  int il = imin;
  while (order(&(*a).e[il], &(*a).e[jmin]) <= 0 && il < imax)
  ...
  if (il < imax) merge_aarray_double_rec(a, e, c, jmin, jmax, il, imax, order);
  else
    {
      int ir = jmin;
      while (ir < jmax)
...
{
  if (order(&(*a).e[imin], &(*a).e[jmin]) <= 0) merge_aarray_double_rec(a, e, c, imin, imax, jmin, jmax, order);
  else merge_aarray_double_rec(a, e, c, jmin, jmax, imin, imax, order);
}
...
void merge_aarray_char_rec(aarray_char *a, array_char *e, int *c, int imin, int imax, int jmin, int jmax, char (*order)(array_char *, array_char *))
{
  if (order(&(*a).e[imin], &(*a).e[jmin]) <= 0) {}
  else
    {
      printf("Incorrect usage! (merge_array_char_rec)\n");
      exit(EXIT_FAILURE);
    }
  int il = imin;
  while (order(&(*a).e[il], &(*a).e[jmin]) <= 0 && il < imax)
  ...
  if (il < imax) merge_aarray_char_rec(a, e, c, jmin, jmax, il, imax, order);
  else
    {
      int ir = jmin;
      while (ir < jmax)
...
{
  if (order(&(*a).e[imin], &(*a).e[jmin]) <= 0) merge_aarray_char_rec(a, e, c, imin, imax, jmin, jmax, order);
  else merge_aarray_char_rec(a, e, c, jmin, jmax, imin, imax, order);
}
...
void array_bit_int_set(array_bit *abit, int bit_pos, char bit_val)
{
  if ((*abit).mem > bit_pos) {}
  else
    {
      ...
      while ((*abit).len <= bit_pos) add2array_bit_int(abit, 0);
    }
  ...
  while (changed)
    {
      ...
      if (mid_comp == 1) llim = mid;
      else if (mid_comp == -1) ulim = mid;
      else return mid;
...
void free_hash_table_int_str_except_elem(hash_table_int_str *ht, void (*free_value_func)(void *))
{
  if (free_value_func != NULL)
    {
      for (int i = 0; i < (*ht).values.len; i++) free_value_func((*ht).values.e[i]);
...
char remove_hash_table_int_str(hash_table_int_str *ht, int *n, int len, void (*free_value_func)(void *))
{
  ...
  if (present)
    {
      remove_hash_table_int_str_exp(ht, elem_addr, free_value_func);
