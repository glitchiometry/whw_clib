...
void free_array_bit_int(void *abit);
...
void free_edge_wtd_graph(void *g);
...
typedef struct
{
	...
	array_int map;
	aarray_int fibers;
	array_int fiber_addr;
} contr_nbrlist;

void contr_nbrlist_init(contr_nbrlist *cnb, nbrlist *top, aarray_int *wts);
void free_contr_nbrlist(void *cnb);
void transcribe_contr_nbrlist(contr_nbrlist *src, contr_nbrlist *b);
...
typedef struct
{
	// The hash function and its associated parameters
	int (*hf)(void *, void *);
	void *hf_params;
	int (*compare_func)(void *, void *); // Used to determine equality, and also for sorting
	void (*copy_func)(void *, void *); // In case the table is self-sourced (i.e. it needs to store its own copy 
					   // of input data, as when the data is generated by an iterator.) This function
					   // would also be invoked (when it is defined) for deep transcriptions/copies.
	// The hash table data (an 'aarray_int' would be slightly easier to use here, but it would also have a 
	// 	slightly higher memory cost.)
 	array_voidstar data;
	array_int addr_0;
	array_int addr_1;
	// The individual elements of the hash table
	array_voidstar elem;
	char lg_data_len; // Number of bits used for hash addresses
	int largest_bin;
	void *key_data_src;
} hash_table_generic;
...
void insert_array_voidstar(array_voidstar *a, void *i, int n); // EDITED
...
void remove_array_voidstar_sorted(array_voidstar *a, int n, void (*free_elem)(void *)); // EDITED
...
void insert_array_char(array_char *a, char c, int i);
...
void free_aarray_double(void *aa);
void print_aarray_double(aarray_double aa);
...
// RESUME: implement these!
// void hash_table_generic_init(hash_table_generic *ht) // This would probably vary depending on the context
void free_hash_table_generic(hash_table_generic *ht, void (*free_func)(void *));
//void resize_hash_table_generic(hash_table_generic *ht, int new_min_size); // This would require a special function to update the parameters of the hashing function
void add2hash_table_generic(hash_table_generic *ht, void *new_elem);
char query_hash_table_generic(hash_table_generic *ht, void *elem, int *addr0, int *addr1);
void remove_hash_table_generic(hash_table_generic *ht, void *elem);
void transcribe_hash_table_generic(hash_table_generic *src, hash_table_generic *dest); 
// RESUME: implement the above!

// RESUME: incorporate actual 'value' data in both hash table implementations
void hash_table_int_init(hash_table_int *ht, int min_data_size, basics_Uint64 max_elem_size);
void free_hash_table_int(hash_table_int *ht, void (*free_elem_data)(void *));
void resize_hash_table_int(hash_table_int *ht, int new_min_size);
void add2hash_table_int(hash_table_int *ht, int key, void *value);
char query_hash_table_int(hash_table_int *ht, int n, int *elem_addr, void **val);
char remove_hash_table_int(hash_table_int *ht, int n, void (*free_value_data)(void *));
...
void hash_table_int_str_init(hash_table_int_str *ht, int min_ht_size, basics_Uint64 max_int_size, char key_data_src_mode);
void free_hash_table_int_str(hash_table_int_str *ht, void (*free_value_data)(void *));
...
char remove_hash_table_int_str(hash_table_int_str *ht, int *n, int len, void (*free_elem_data)(void *));
...
typedef struct
{
	array_int membership;
	aarray_int clusters;
	array_int cluster_addr;
	array_int cluster_size;
} union_find;

void union_find_init(union_find *uf, int size);
void free_union_find(void *uf);
